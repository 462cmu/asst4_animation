 ...
 *
 * - When the timeline is playing, it will advance by 1 timestep at every 'step' call.
 * - Important times made be arbitrarily added and removed. (a.k.a 'marked times')
 *
 *
...
namespace CMU462 {
   ...
   class Timeline{
      ...
      public:
         ...
         Timeline() : x(0), y(0), w(64*6), h(0){}

         Timeline(int x, int y, int width, int height) : x(x), y(y), w(width), h(height)
         {
		   current_frame = 0;
		   max_frame     = 1;
		   isPlaying    = false;
		 ...
		 // Returns true iff the given mouse location is within the bounds of this timeline.
		 // This function sets the particular button containing (x, y) to the hover state.
		 ...
		 // make the timeline longer or shorter by the given number of frames
                 // (note that if there are events past the end of the timeline, they will remain!)
