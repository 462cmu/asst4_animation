  ...
  while( elem ) {
    ...
    if( elementType == "line" ) {
      ...
      svg->elements.push_back( image );
       ...
       Path* path = new Path();
       parseElement( elem, path);
       parsePath( elem, path );// Recursion.
       svg->elements.push_back( path );

    } else {
       // unknown element type --- include default handler here if desired
	  cout << "WARNING: Type: " << elementType << " is not handled!";
    }
...
void SVGParser::parseElement( XMLElement* xml, SVGElement* element ) {
  ...
  if( opacity ) style->fillColor.a = atof( opacity );
...
void SVGParser::parseCircle( XMLElement* xml, Circle* circle) {
  circle->center = Vector2D(xml->FloatAttribute( "cx" ),
                            xml->FloatAttribute( "cy" ));
...
void SVGParser::parsePath( XMLElement* xml, Path* path) {

  // FIXME : Parse the 'd' list of spatial data from Paths.
...
double strokeStyleToMassDensity( const Style& s )
{
   ...
   return baseLineDensity * s.strokeColor.a * ( 1. + strokeWidthDensityFactor*( s.strokeWidth - 1. ));
}
...
double Path :: mass( void ) const
{
   // FIXME compute actual mass for a curved path
   return 1.;
}

double Path :: momentOfInertia( const Vector2D& center ) const
{
   // FIXME compute actual moment of inertia for a curved path
   return 1.;
}

Vector2D Path :: centroid( void ) const
{
   // FIXME compute actual centroid for a curved path
   return Vector2D( 0., 0. );
}
...
Vector2D Point :: centroid( void ) const
{
   return position;
}
...
Vector2D Line :: centroid( void ) const
{
   return ( from + to ) / 2.;
}
...
Vector2D Polygon :: centroid( void ) const
{
...
Vector2D Ellipse :: centroid( void ) const
{
   return center;
}
...
Vector2D Circle :: centroid( void ) const
{
   return center;
}
