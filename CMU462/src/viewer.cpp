...
void Viewer::init() {
  ...
  if (application){
    if (HDPI) application->use_hdpi_reneder_target();
    application->init();
  } 
  ...
  line_id_application = osd_text->add_line(-0.95,  0.90, "Application", 
...
void Viewer::resize_callback( GLFWwindow* window, int width, int height ) {
  ...
  if (application) application->resize( buffer_w, buffer_h );  
}
